    ...
    <section ref={ref} className="relative min-h-[85vh] flex items-center overflow-hidden">
      ...
      <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 md:py-36">
        <div className="max-w-3xl">
          <div className={`inline-flex items-center gap-2 bg-blue-500/20 border border-blue-400/30 backdrop-blur-sm rounded-full px-4 py-1.5 text-sm text-blue-200 mb-8 shadow-sm shadow-blue-500/10 opacity-0 ${inView ? 'animate-slide-up' : ''}`} style={{ animationDelay: '0ms' }}>
            <Shield className="h-4 w-4" />
            Trusted Logistics Partner Since 2000
          </div>
          <h1 className={`text-4xl md:text-7xl font-bold leading-[1.05] tracking-tight mb-6 text-balance text-white opacity-0 ${inView ? 'animate-slide-up' : ''}`} style={{ animationDelay: '150ms' }}>
            Moving Your Business
            <span className="block text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-cyan-200">
              Across Borders
            </span>
          </h1>
          <p className={`text-lg md:text-xl text-blue-100/80 mb-10 leading-relaxed max-w-2xl opacity-0 ${inView ? 'animate-slide-up' : ''}`} style={{ animationDelay: '300ms' }}>
          ...
          <div className={`flex flex-wrap gap-4 opacity-0 ${inView ? 'animate-slide-up' : ''}`} style={{ animationDelay: '450ms' }}>
            <Link
              to="/services"
